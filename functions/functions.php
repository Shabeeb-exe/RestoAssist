...
if (isset($_POST['login'])) {
    ...
    $query = "select * from tbl_staffs where user='$user' and pass='$pass'";
...
if (isset($_POST["add_staff"])) {
    ...
    $query = $con->query("insert into tbl_staffs(staff_id, img, name, phone, mail, desg, dob, user, pass, user_type) values('$staff_id', '$new_file_name', '$name', '$phone', '$mail', '$desg', '$dob', '$user', '$pass', 'staff')");
...
if (isset($_POST["update_staff"])) {
    ...
    $query = $con->query("update tbl_staffs set img='$new_file_name', name='$name', desg='$desg', phone='$phone', mail='$mail', dob='$dob', user='$user', pass='$pass' where staff_id='$sid'");
...
if (isset($_POST["del_staff"])) {
    ...
    $query = $con->query("delete from tbl_staffs where staff_id='$id'");

    if ($query === TRUE) {
        header("location:../staffs-manage?success=Staff deleted successfully");
    } else {
        header("location:../staffs-manage?error=Failed to delete staff!");
    }
}
...
if (isset($_POST["change-pass"])) {
    ...
    if ($npass == $rnpass) {
        ...
        $query = "select * from tbl_staffs where user='$user_id' and pass='$opass'";
        $run = mysqli_query($con, $query);
        $count = mysqli_num_rows($run);

        if ($count == 1) {
            $query = $con->query("update tbl_staffs set pass='$npass' where user='$user_id'");
            if ($query == TRUE) {
